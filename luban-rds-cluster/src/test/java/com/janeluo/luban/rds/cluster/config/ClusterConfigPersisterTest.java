...
public class ClusterConfigPersisterTest {
    ...
    // ==================== 第四批（N-29：vars 段 / 真实时间戳 / 迁移方括号） ====================

    @Test
    public void testSaveWritesRealRedisVarsSection() throws IOException {
        ClusterConfig config = createTestConfig();
        config.setLastVoteEpoch(7);

        persister.save(config, tempFile.getAbsolutePath());

        String persisted = new String(Files.readAllBytes(tempFile.toPath()));
        assertTrue("nodes.conf 应包含真实 Redis 可解析的 vars 段",
                persisted.contains("vars currentEpoch 10 lastVoteEpoch 7"));

        ClusterConfig loadedConfig = persister.load(tempFile.getAbsolutePath());
        assertEquals("vars currentEpoch 应恢复", 10, loadedConfig.getCurrentEpoch());
        assertEquals("vars lastVoteEpoch 应恢复", 7, loadedConfig.getLastVoteEpoch());
    }

    @Test
    public void testLoadParsesRealRedisVarsLine() throws IOException {
        // 真实 Redis 生成的 nodes.conf vars 段（无本实现的注释行）
        Files.write(tempFile.toPath(), java.util.Arrays.asList(
                "# Generated by Redis",
                "vars currentEpoch 42 lastVoteEpoch 13",
                "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0 127.0.0.1:7000@17000 master,myself - 0 1722600000000 5 connected 0-100"
        ));

        ClusterConfig loadedConfig = persister.load(tempFile.getAbsolutePath());

        assertEquals("真实 Redis vars 段 currentEpoch 应被解析", 42, loadedConfig.getCurrentEpoch());
        assertEquals("真实 Redis vars 段 lastVoteEpoch 应被解析", 13, loadedConfig.getLastVoteEpoch());
    }

    @Test
    public void testSaveWritesRealPingPongTimestamps() throws IOException {
        ClusterConfig config = createTestConfig();
        ClusterNode node = config.getNode("a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0");
        node.setLastPingTime(1722600001000L);
        node.setLastPongTime(1722600002000L);

        persister.save(config, tempFile.getAbsolutePath());

        String persisted = new String(Files.readAllBytes(tempFile.toPath()));
        assertTrue("nodes.conf 应写真实 ping 时间戳", persisted.contains("1722600001000"));
        assertTrue("nodes.conf 应写真实 pong 时间戳", persisted.contains("1722600002000"));

        ClusterConfig loadedConfig = persister.load(tempFile.getAbsolutePath());
        ClusterNode loadedNode = loadedConfig.getNode(node.getNodeId());
        assertEquals("ping 时间戳应往返恢复", 1722600001000L, loadedNode.getLastPingTime());
        assertEquals("pong 时间戳应往返恢复", 1722600002000L, loadedNode.getLastPongTime());
    }

    @Test
    public void testSaveAndLoadSlotMigrationBrackets() throws IOException {
        ClusterConfig config = createTestConfig();
        ClusterNode node = config.getNode("a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0");
        node.addSlot(100);
        node.addSlot(101);
        config.setSlotOwner(100, node.getNodeId());
        config.setSlotOwner(101, node.getNodeId());
        // 迁移方括号：100 迁移到目标，101 从源导入（对齐 Redis [slot->-id] / [slot-<-id]）
        config.setSlotMigrating(100, "cccccccccccccccccccccccccccccccccccccccc");
        config.setSlotImporting(101, "dddddddddddddddddddddddddddddddddddddddd");

        persister.save(config, tempFile.getAbsolutePath());

        String persisted = new String(Files.readAllBytes(tempFile.toPath()));
        assertTrue("nodes.conf 应输出迁移方括号 [100->-...]",
                persisted.contains("[100->-cccccccccccccccccccccccccccccccccccccccc]"));
        assertTrue("nodes.conf 应输出导入方括号 [101-<-...]",
                persisted.contains("[101-<-dddddddddddddddddddddddddddddddddddddddd]"));

        ClusterConfig loadedConfig = persister.load(tempFile.getAbsolutePath());
        assertEquals("迁移状态应往返恢复", "cccccccccccccccccccccccccccccccccccccccc",
                loadedConfig.getMigratingTarget(100));
        assertEquals("导入状态应往返恢复", "dddddddddddddddddddddddddddddddddddddddd",
                loadedConfig.getImportingSource(101));
    }

    @Test
    public void testLoadParsesRealRedisMigrationBrackets() throws IOException {
        // 真实 Redis nodes.conf 的方括号写法（含范围形式）
        Files.write(tempFile.toPath(), java.util.Arrays.asList(
                "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0 127.0.0.1:7000@17000 master,myself - 0 1722600000000 5 connected 0-100 [50-60->-cccccccccccccccccccccccccccccccccccccccc] [80-<-dddddddddddddddddddddddddddddddddddddddd]"
        ));

        ClusterConfig loadedConfig = persister.load(tempFile.getAbsolutePath());

        ClusterNode loadedNode = loadedConfig.getNode("a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0");
        assertNotNull(loadedNode);
        assertTrue("方括号槽位不应影响正常槽位解析", loadedNode.getSlotCount() >= 101);
        assertEquals("范围迁移状态应解析", "cccccccccccccccccccccccccccccccccccccccc",
                loadedConfig.getMigratingTarget(55));
        assertNull("范围外槽位不应有迁移状态", loadedConfig.getMigratingTarget(49));
        assertEquals("范围导入状态应解析", "dddddddddddddddddddddddddddddddddddddddd",
                loadedConfig.getImportingSource(80));
    }
