...
Luban-RDS 使用 LuaJ 引擎实现了完整的 Lua 脚本支持，与 Redis 完全兼容。本部分详细介绍 Lua 脚本的使用方法和 API 参考。
...
- **脚本缓存**：通过 SHA1 值执行缓存的脚本，提高执行效率
- **原子性执行**：脚本执行期间不会被其他命令中断
- **沙箱模式**：支持限制 Lua 模块和函数的访问，保障安全
- **脚本超时**：支持脚本执行超时设置，防止长时间阻塞
- **内存限制**：支持脚本大小和返回值大小限制
...
1. **脚本执行时间**：避免执行耗时过长的脚本，默认超时 10 秒
2. **内存使用**：注意脚本大小和返回值大小限制
3. **安全性**：避免在脚本中执行危险操作，建议启用沙箱模式
