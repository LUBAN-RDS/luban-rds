...
Luban-RDS 采用三层线程模型，实现高性能的并发处理：

| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| io-threads | 整数 | CPU 核心数 | Boss Group 线程数，负责接受新连接 |
| worker-threads | 整数 | CPU 核心数 * 2 | Worker Group 线程数，负责 I/O 读写操作 |
| business-threads | 整数 | CPU 核心数 | 业务线程池大小，负责执行命令逻辑 |
...
│              处理业务逻辑命令，避免阻塞 I/O 线程                │
└─────────────────────────────────────────────────────────────┘
```

**配置建议**：
- **io-threads**：通常设置为 1-2 即可，因为接受连接的开销很小
- **worker-threads**：建议设置为 CPU 核心数的 2 倍，充分利用多核
- **business-threads**：根据业务复杂度调整，CPU 密集型设置为 CPU 核心数，I/O 密集型可适当增加
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| expire-cycle-interval | 整数 | 100 | 过期键主动清理间隔（毫秒） |
| expire-cycle-keys | 整数 | 100 | 每次清理的最大过期键数量 |

**过期键清理策略说明**：
- **惰性删除**：访问键时检查是否过期，过期则删除
- **主动清理**：后台定时任务扫描并清理过期键，避免过期键长期占用内存
- **清理限制**：每次清理限制键数量，避免阻塞主线程
...
**并发安全特性**：
- **分段锁机制**：使用 1024 个分段锁保证并发安全，避免内存泄漏
- **原子性操作**：MSET 等批量操作保证原子性
- **竞态条件处理**：过期键检查使用双重检查锁定机制

### 4.4 内存池配置

Luban-RDS 集成 Netty 内存池，优化网络缓冲区的内存分配：

| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| use-pool | 布尔值 | yes | 是否使用内存池，启用后可减少 GC 压力 |
| leak-detection | 字符串 | simple | 内存泄漏检测级别 |

**内存泄漏检测级别**：
- `disabled`：禁用检测（生产环境推荐）
- `simple`：简单检测（默认，开发环境推荐）
- `advanced`：高级检测
- `paranoid`：偏执模式（调试内存问题时使用）
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| memory-frag-threshold | 整数 | 30 | 内存碎片率阈值（百分比），超过此值自动触发碎片整理 |
...
```bash
# 执行内存碎片整理
redis-cli -h localhost -p 9736 MEMORY PURGE
...
used_memory_peak_perc:52.63%
mem_fragmentation_ratio:15.5%
total_keys:1000
expired_keys:50
```
