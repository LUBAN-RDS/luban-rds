...
| 模块 | 职责 | 关键组件 |
|------|------|----------|
| **luban-rds-core** | 核心业务逻辑 | `CommandHandler`, `MemoryStore`, `LuaCommandHandler` |
| **luban-rds-protocol** | 协议解析 | `RedisProtocolParser`, `Command`, `RespType` |
| **luban-rds-server** | 网络服务 | `NettyRedisServer`, `RedisServerHandler`, `PubSubManager`, `LubanInfoProvider` |
...
- **基于 Netty 4**：使用 NIO 非阻塞 IO，支持高并发
...
| 处理器 | 处理的命令 |
|--------|------------|
| **StringCommandHandler** | SET, GET, INCR, DECR, INCRBY, DECRBY, APPEND, STRLEN |
| **HashCommandHandler** | HSET, HGET, HGETALL, HDEL, HEXISTS, HKEYS, HVALS, HLEN |
| **ListCommandHandler** | LPUSH, RPUSH, LPOP, RPOP, LLEN, LRANGE |
| **SetCommandHandler** | SADD, SREM, SMEMBERS, SISMEMBER, SCARD |
| **ZSetCommandHandler** | ZADD, ZRANGE, ZSCORE, ZREM, ZCARD |
| **CommonCommandHandler** | EXISTS, DEL, EXPIRE, TTL, FLUSHALL, TYPE, ECHO, SELECT, INFO, SCAN, DBSIZE, FLUSHDB, TIME, LASTSAVE, BGREWRITEAOF, BGSAVE |
